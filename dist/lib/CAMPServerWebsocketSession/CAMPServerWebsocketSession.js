...
import { CAMPFrameType, BufferUtil, ByeFrame, EndpointInfoFrame, } from "camp-protocol";
...
export class CAMPServerWebsocketSession extends EventEmitter {
    webSocket;
    tcpSocket;
    remoteName;
    extensionRegistry;
    ...
    constructor(webSocket, tcpSocket, remoteName, backpressure_opts, extensionRegistry) {
        ...
        this.base = new CAMPBaseManager(this.sid, this.bind(this.send), this.bind(this.next_ack), this.bind(this.Destroy), (features) => this.receivedProtocolFeatures = features, this.extensionRegistry.get_executor(this), this.client_ack_tracker, () => this.webSocket.isAlive = true);
        ...
        //Handler first
        this.base.on("ready", () => {
            this.stream = new CAMPTransactionManager(this.sid, this.bind(this.send), this.bind(this.next_ack), this.bind(this.next_txid), this.bind(this.Destroy), () => this.receivedProtocolFeatures, this.bp_mgr.waitUntilEmpty.bind(this.bp_mgr));
            this.emit("connected");
        });
